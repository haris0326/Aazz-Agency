{{--
    ...
    Save as: resources/views/blog_show.blade.php
    Served by: BlogController@show -> route('blog.show')
    URL: domain.com/blog/{slug}
    ============================================================
--}}
@extends(config('web_assets.layouts.main'))

@section('title', ($blog->seo->meta_title ?? $blog->title) . ' | ' . config('app.name', 'Aazz Agency'))
@section('description', \Illuminate\Support\Str::limit(strip_tags($blog->seo->meta_description ?? $blog->excerpt ?? strip_tags($blog->content)), 160))
@section('canonical_url', $blog->seo->canonical_url ?? route('blog.show', $blog->slug))
@section('og_image', $blog->seo->og_image ?? ($blog->featured_image ? asset($blog->featured_image) : asset('web_assets/images/default-og.png')))
        ...
        <div class="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12">
            ...
            <article class="w-full lg:w-8/12">
                ...
                <div class="mt-8 flex flex-wrap items-center gap-3">
                    ...
                    @php $shareUrl = urlencode(route('blog.show', $blog->slug)); @endphp
                    ...
                    <button type="button" id="copyLinkBtn" data-url="{{ route('blog.show', $blog->slug) }}"
                ...
                <div class="mt-12 p-6 md:p-8 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-5 reveal-up">
                    ...
                    <div>
                        ...
                        <h4 class="text-lg font-bold text-slate-900">{{ $blog->author->name ?? 'Aazz Agency Team' }}</h4>
                        <p class="text-slate-500 text-sm mt-1">Sharing insights on digital growth, technology and strategy.</p>
                    </div>
                </div>

                {{-- ============================== --}}
                {{-- COMMENTS (wired to real backend) --}}
                {{-- ============================== --}}
                <div class="mt-16 pt-12 border-t border-slate-100">
                    <h3 class="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3">
                        ...
                        Comments <span id="commentCount" class="text-slate-400 font-semibold text-lg">({{ $comments->count() }})</span>
                    </h3>
                        ...
                        @forelse($comments as $c)
                            <div class="flex gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                <div class="w-11 h-11 shrink-0 rounded-full bg-gradient-to-br from-blue-600 to-teal-400 flex items-center justify-center text-white font-bold text-sm uppercase">
                                    {{ \Illuminate\Support\Str::substr($c->name, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <div class="flex items-center gap-3 mb-1">
                                        <p class="font-bold text-slate-900">{{ $c->name }}</p>
                                        <span class="text-xs text-slate-400">{{ $c->created_at->format('M j, Y') }}</span>
                                    </div>
                    ...
                    <div class="bg-slate-50 rounded-[2rem] p-6 md:p-8 border border-slate-100">
                        ...
                        <form id="blogCommentForm" novalidate
                              action="{{ route('blog.comments.store', $blog->id) }}"
                              method="POST">
                            @csrf
                ...
                <div class="lg:sticky lg:top-28 space-y-6">
                    ...
                    <div class="bg-[#020617] rounded-[1.75rem] p-6 overflow-hidden relative">
                        ...
                        <h4 class="text-sm font-black uppercase tracking-widest text-teal-400 mb-5 flex items-center gap-2 relative z-10">
                            ...
                            {{ $blog->category->name ?? 'Related' }} Posts
                        </h4>
                        <div class="space-y-4 relative z-10">
                            @foreach($relatedBlogs as $related)
                                <a href="{{ route('blog.show', $related->slug) }}" class="flex items-center gap-3 group">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-slate-500 text-xs mt-1">
                                            {{ ($related->published_at ?? $related->created_at)?->format('M j, Y') }}
                                        </p>
                                    </div>
                                </a>
                            @endforeach
            ...
            @foreach($relatedBlogs->take(3) as $related)
                <a href="{{ route('blog.show', $related->slug) }}"
                   class="related-card group bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition duration-500 transform hover:-translate-y-2 reveal-up">
                    ...
                    <div class="p-6">
                        @if($related->category)
                            <p class="text-xs text-teal-600 font-bold uppercase tracking-widest mb-2">{{ $related->category->name }}</p>
                        @endif
...
<script type="application/ld+json">
{
  ...
  "description": {!! json_encode(\Illuminate\Support\Str::limit(strip_tags($blog->seo->meta_description ?? $blog->excerpt ?? ''), 160)) !!},
  "image": {!! json_encode($blog->featured_image ? asset($blog->featured_image) : asset('web_assets/images/default-og.png')) !!},
  "datePublished": {!! json_encode(optional($blog->published_at ?? $blog->created_at)->toIso8601String()) !!},
  "dateModified": {!! json_encode(optional($blog->updated_at)->toIso8601String()) !!},
  "author": { "@type": "Person", "name": {!! json_encode($blog->author->name ?? 'Aazz Agency') !!} },
  "mainEntityOfPage": { "@type": "WebPage", "@id": {!! json_encode(route('blog.show', $blog->slug)) !!} }
}
</script>
