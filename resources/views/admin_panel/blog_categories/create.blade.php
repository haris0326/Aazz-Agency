...
<x-admin.page-header
    ...
    subtitle="Create a new category to organize your blog posts"
    :breadcrumbs="[['label' => 'Dashboard', 'url' => route('admin.panel')], ['label' => 'Blog', 'url' => route('blog.index')], ['label' => 'Categories', 'url' => route('blog-categories.index')], ['label' => 'Add New']]"
/>
...
<form action="{{ route('blog-categories.store') }}" method="POST">
    ...
    <x-admin.form-section icon="bi-tag" title="Category Details">
        <x-admin.field type="text" name="name" label="Category Name" icon="bi-type" :value="old('name')" required />
    ...
    <div class="ap-form-actions" style="position: static; margin: 0; border-radius: var(--ap-radius); border: 1px solid var(--ap-border);">
        ...
        <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg"></i> Save Category</button>
    </div>
</form>
