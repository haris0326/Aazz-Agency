...
<section class="py-24 bg-[#020617] overflow-hidden">
    <div class="container mx-auto px-6">
        <h2 class="text-2xl font-black text-white mb-12 flex items-center gap-6">
            Explore More Cities <span class="h-px flex-1 bg-white/10"></span>
        </h2>
        <div id="city-slider" class="city-slider-container flex overflow-x-auto hide-scrollbar gap-4 pb-10">
            @foreach($cities as $cityItem)
            <a href="{{ route('show_pkg', ['pkg_cat' => $category->slug, 'city' => $cityItem->slug]) }}" 
               class="whitespace-nowrap px-8 py-5 bg-white/5 border border-white/10 rounded-2xl text-slate-300 hover:text-white hover:border-teal-400 hover:bg-teal-400/10 transition-all text-sm font-bold shadow-lg">
                <i class="fas fa-map-marker-alt text-teal-400 mr-2"></i> {{ ucfirst($cityItem->name) }}
            </a>
            @endforeach
        </div>
    </div>
